/**
 * Türkiye'nin 81 ili, plaka koduyla birlikte.
 *
 * Veride ilin adı değil **plaka kodu** saklanır: ad yazımı değişebilir (Hakkâri/Hakkari gibi)
 * ama kod sabittir ve tamsayı olarak karşılaştırılması kolaydır.
 *
 * Kullanım: Sonuç ekranındaki isteğe bağlı bilgi formu (il listesi) ve kaydedilen sonucun
 * doğrulanması (kodun 1-81 aralığında ve listede bulunması).
 */

export interface Province {
  /** Plaka kodu (1-81). Veritabanında saklanan değer budur. */
  code: number;
  name: string;
}
...
/** Plaka kodundan il adını verir; kod listede yoksa `undefined`. */
export function getProvinceName(code: number): string | undefined {
  return PROVINCES.find((province) => province.code === code)?.name;
}

/** Verilen kodun geçerli bir plaka kodu olup olmadığını söyler. */
